...
const isMainModule = !isCloudEnvironment && mainModulePath && (
...
// Log for debugging
logger.info(`Main module check: mainPath=${mainModulePath}, currentPath=${currentFilePath}, isMain=${isMainModule}, isCloudEnv=${isCloudEnvironment}`);
...
if (isMainModule) {
  ...
  const tryStartServer = async () => {
    ...
    logger.error(`❌ Could not find an available port after ${maxAttempts} attempts.`);
    process.exit(1);
  };
